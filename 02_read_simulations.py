...
from simulation_settings import parameter_range, simulation_types, \
    ...
    p_goal_range, p_continuity_range, p_social_range, p_memory_range, \
    p_memory_range_used, sd_goal_range, sd_continuity, sd_social, \
...
var_shape = (len(p_goal_range), len(p_social_range), \
    len(p_memory_range_used), len(sd_goal_range), len(sd_memory_max_range),  \
...
for dir_count, dir_name in enumerate(all_data_directories):
    ...
    pci = p_continuity_range.index(param["Pcontinuity"])
    pmi = p_memory_range_used.index(param["Pmemory"])
    ...
    for sub_dir_count, sub_dir_name in enumerate(sub_directory_names):
        ...
        print("\tProcessing '{}' ({}/{}) with {} files".format(sub_dir_name, \
            sub_dir_count+1, n_sub_directories, len(sub_dir_files)))
