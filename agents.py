...
class Agent:
    ...
    def __init__(self, starting_heading=0.0, starting_position=(0.0, 0.0), \
        ...
        landmark_threshold=10.0, n_landmarks=20, prefer_first_path=True):
    ...
    def advance_position(self, other_position=None, force_position=None, \
        ...
        if not self._finished:
            ...
            # Update the heading and position if we're not there yet.
                ...
                if force_position != None:
                    ...
                    h_components = self.update_heading( \
        ...
        if return_components:
            return self.get_position(), self._finished, h_components
        else:
    ...
    def sample_memory_heading(self, no_error=False):
        ...
        # Only rely on memory if there are any paths.
        n_memorised_paths = len(self._path_history)
        if n_memorised_paths == 0:
        ...
        # Find the heading towards the nearest next landmark.
            ...
            if self._current_landmark >= self._path_history[0].shape[1]:
                ref_point = self._goal
            else:
                ref_point = self._path_history[0][:,self._current_landmark]
            ...
            if self._current_landmark >= self._path_history[0].shape[1]:
                ...
                next_point = self._path_history[0][:,self._current_landmark]
            # If memory precision is not constant, choose the most
            # appropriate SD.
            if type(self._memory_kappa) == list:
                if n_memorised_paths >= len(self._memory_kappa):
                    kappa = self._memory_kappa[-1]
                else:
                    kappa = self._memory_kappa[n_memorised_paths-1]
            else:
    ...
    def update_heading(self, other_position=None):
